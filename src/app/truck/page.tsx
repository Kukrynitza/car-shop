...
export default function Page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setBrandExist(false)
      const truckBrand = (otherBrand)
        ? await selectAllTruckBrand()
        : await selectFirstTruckBrand()
  ...
  const [
    ...
    async function getAnns() {
      ...
      const anns = await selectAnnouncements('truck', sortSelect.value, currentPage, inputInfo, activeSortData, brands)
      const count = await selectAnnouncementsCount('truck', inputInfo, activeSortData, brands)
      setAnnouncements(anns)
      setTotalCount(Number(count[0].count))
    }
    await getAnns()
