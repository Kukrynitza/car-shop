...
import selectAllTruckBrand from '@/actions/Truck/selectAllTruckBrand'
import selectFirstTruckBrand from '@/actions/Truck/selectFirstTruckBrand'
...
export default function Page() {
  ...
  useEffect(() => {
    async function fetchDataAndProcess() {
      ...
      const data = await sortInfo(modelActive, 'truck', ids)
      if (data) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setBrandExist(false)
      const truckBrand = (otherBrand) ? await selectAllTruckBrand() : await selectFirstTruckBrand()
      const truck: BrandForFilter[] = truckBrand.map(
        (element) => (
          {
            active: false,
            count: element.count,
  ...
  useEffect(() => {
    async function getAnnouncementCount() {
      const count = await selectAnnouncementsCount('truck', inputSortInfo, activeSortData, [])
      if (count) {
  ...
  useEffect(() => {
    async function getAnnouncement() {
      ...
      const anns = currentPage
        ? await selectAnnouncements('truck', sortSelect.value, currentPage, inputSortInfo, activeSortData, brands)
        : await selectAnnouncements('truck', sortSelect.value, 1, inputSortInfo, activeSortData, brands)
  ...
  const [
    ...
    async function getAnns() {
      ...
      const anns = await selectAnnouncements('truck', sortSelect.value, currentPage, inputSortInfo, activeSortData, brands)
      const count = await selectAnnouncementsCount('truck', inputSortInfo, activeSortData, brands)
          ...
          <div className={styles.sort}>
            {mounted ? (
              <CustomSelect
                ...
                setChange={(value) => setSortSelect(value!)}
              />
            ) : null}
          </div>
          {largeSort
        ...
        <div className={styles.pagination}>
          ...
          && (
            <span className={styles.span}>
              <Link href={generateUrl(currentPage - 1)}>P</Link>
            </span>
          )}
          {currentPage - 1 > 1
          && (
          ...
          && (
            <span className={styles.span}>
              <Link href={generateUrl(currentPage + 1)}>N</Link>
            </span>
          )}
        </div>
      </div>
