...
import selectAllMotorcycleBrand from '@/actions/Motorcycle/selectAllMotorcycleBrand'
import selectFirstMotorcycleBrand from '@/actions/Motorcycle/selectFirstMotorcycleBrand'
...
export default function Page() {
  ...
  useEffect(() => {
    async function fetchDataAndProcess() {
      ...
      const data = await sortInfo(modelActive, 'motorcycle', ids)
      if (data) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setBrandExist(false)
      const motorcycleBrand = (otherBrand)
        ? await selectAllMotorcycleBrand()
        : await selectFirstMotorcycleBrand()
      const motorcycle: BrandForFilter[] = motorcycleBrand.map(
        (element) => (
          {
            active: false,
            count: element.count,
  ...
  useEffect(() => {
    async function getAnnouncement() {
      ...
      const anns = currentPage
        ? await selectAnnouncements('motorcycle', sortSelect.value, currentPage, inputSortInfo, activeSortData, brands)
        : await selectAnnouncements('motorcycle', sortSelect.value, 1, inputSortInfo, activeSortData, brands)
  ...
  const [
    ...
    async function getAnns() {
      ...
      const anns = await selectAnnouncements('motorcycle', sortSelect.value, currentPage, inputInfo, activeSortData, brands)
      const count = await selectAnnouncementsCount('motorcycle', inputInfo, activeSortData, brands)
      setAnnouncements(anns)
      setTotalCount(Number(count[0].count))
    }
    await getAnns()
